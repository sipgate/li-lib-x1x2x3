...
class TaskFactoryTest {
  ...
  void it_creates_task_from_task_details() {
    // GIVEN
    final var destination = new Destination(D_ID, null, null, null, 0);
    when(destinationRepository.findByDIDs(Set.of(D_ID))).thenReturn(Set.of(destination));
    ...
    // THEN
    assertThat(actual).isEqualTo(new Task(X_ID, Set.of(destination), E_164_NUMBER, DELIVERY_TYPE));
  }
